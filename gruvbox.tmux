...
build_window_icon() {
  ...
  if [ "$window_status_icon_enable" = "none" ]
  then
    local show_window_status=""
  fi
...
build_window_format() {
  ...
  if [ "$fill" = "none" ]
  then
    local show_left_separator="#[fg=$thm_gray,bg=$status_style,nobold,nounderscore,noitalics]$window_left_separator"
    ...
    local show_right_separator="#[fg=$thm_gray,bg=$status_style]$window_right_separator"

  fi

  if [ "$fill" = "all" ]
  then
    local show_left_separator="#[fg=$color,bg=$status_style,nobold,nounderscore,noitalics]$window_left_separator"
    ...
    local show_right_separator="#[fg=$color,bg=$status_style]$window_right_separator"

  fi

  if [ "$fill" = "number" ]
  then
    local show_number="#[fg=$thm_bg,bg=$color]$number"
    ...
    then
      local show_left_separator="#[fg=$background,bg=$status_style,nobold,nounderscore,noitalics]$window_left_separator"
      local show_right_separator="#[fg=$color,bg=$status_style]$window_right_separator"
    fi

    if [ "$window_number_position" = "left" ]
    then
      local show_right_separator="#[fg=$background,bg=$status_style,nobold,nounderscore,noitalics]$window_right_separator"
      local show_left_separator="#[fg=$color,bg=$status_style]$window_left_separator"
    fi
...
build_status_module() {
  ...
  then
    ...
    local show_text="#[fg=$thm_fg,bg=$thm_dark] $text"

    local show_right_separator="#[fg=$thm_dark,bg=$status_style,nobold,nounderscore,noitalics]$status_right_separator"
    ...
    then
      ...
      local show_left_separator="#[fg=$color,bg=$status_style,nobold,nounderscore,noitalics]$status_left_separator"
      local show_right_separator="#[fg=$thm_dark,bg=$status_style,nobold,nounderscore,noitalics]$status_right_separator"
    fi
  ...
  then
    ...
    local show_icon="#[fg=$status_style,bg=$color,nobold,nounderscore,noitalics]$icon "
    local show_text="#[fg=$status_style,bg=$color]$text"
    ...
    then
      ...
      local show_left_separator="#[fg=$color,bg=$status_style,nobold,nounderscore,noitalics]$status_left_separator"
      local show_right_separator="#[fg=$color,bg=$status_style,nobold,nounderscore,noitalics]$status_right_separator"
    fi
  ...
  then
    ...
    then
      ...
      local show_right_separator="#[fg=$status_style,bg=$color,nobold,nounderscore,noitalics]$status_right_separator"
    fi
  fi
...
main() {
  ...
  # get status style
  local transparent_status_bar="$(get_tmux_option "@gruvbox_transparent_status_bar" "no")"
  local status_style="${thm_bg}"

  if [ "$transparent_status_bar" = "yes" ]; then
  ...
  set pane-active-border-style "fg=${thm_beige}"

  # windows
  setw window-status-activity-style "fg=${thm_fg},bg=${status_style},none"
  setw window-status-separator ""
  setw window-status-style "fg=${thm_fg},bg=${status_style},none"

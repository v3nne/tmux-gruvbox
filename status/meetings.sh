show_meetings() {
  ...
  local text
  local module
  icon="$(get_tmux_option "@gruvbox_meetings_icon" "")"
  color="$(get_tmux_option "@gruvbox_meetings_color" "$thm_blue")"
  result="$(main)"
  text="$(get_tmux_option "@gruvbox_meetings_text" "$result")"
...
get_attendees() {
  attendees=$(
    icalBuddy \
      ...
      --excludeEndDates \
      --bullet "" \
      --excludeCals "butler.alex2010@gmail.com" \
      eventsToday
  )
}
